...
for current_taks in Entry_list:
    ...
    for type in input_type:
        ...
        if current_taks == ' Daily ':
            ...
            today_history_data_query_cmd = "list_%s_zbjl.select().where(list_%s_zbjl.time_update.endswith('History'),list_%s_zbjl.time_update.startwith('%s')).order_by(list_%s_zbjl.livestraming_time).limit(10)" % (type, type, today_date, type)
