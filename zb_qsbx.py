    ...
    for one_record in eval('list_' + type + '.select()'):
        ...
        while 1:
            try:
                ...
                print('[*] Get zb_qsbx trend_url failed. type:%s, num_zb:%s, url_zb:%s at %s' % (type, one_record.num_zb, one_record.url_zb, time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())))
                time.sleep(5)
            else:
                break
        ...
        for item in data:
            ...
            Table_obj_fans.time_update = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
            ...
            Table_obj_supports.supports_time = item.get('rank_date')
            Table_obj_supports.support_zl = item.get('total_favorited')
            Table_obj_supports.time_update = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
            ...
            Table_obj_works.works_time = item.get('rank_date')
            Table_obj_works.works_zl = item.get('aweme_count')
            Table_obj_works.time_update = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())

            Table_obj_fans.save()
            Table_obj_supports.save()
            Table_obj_works.save()

        print('[+]',type,'zb_qsbx',one_record.num_zb,one_record.name_zb,'Done at',time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))