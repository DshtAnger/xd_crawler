...
for current_taks in Entry_list:
    ...
    for type in input_type:
        ...
        if current_taks == ' Daily ':
            ...
            all_history_data_query_cmd = "list_%s_zbjl_zzgm.select().where(list_%s_zbjl_zzgm.time_update.endswith('History')).order_by(list_%s_zbjl_zzgm.livestraming_time).limit(10)" % (type, type, type)
            today_history_data_query_cmd = "list_%s_zbjl_zzgm.select().where(list_%s_zbjl_zzgm.time_update.endswith('History'),list_%s_zbjl_zzgm.time_update.startswith('%s')).order_by(list_%s_zbjl_zzgm.livestraming_time).limit(10)" % (type, type, type, today_date, type)
        ...
        for one_record in eval(query_cmd):
            ...
            logging.info(' '.join(['[%s]'%current_taks, type, 'zbjl_zzgm', one_record.num_zb, one_record.name_zb, webcast_id, one_record.livestraming_time, '[ zbjl_zzgm amount: %d ]'%zzgm_count, 'Done at', get_current_time()]))
