...
for type in input_type:
    ...
    for one_record in eval('list_' + type + '.select()'):
        ...
        while finish_get_awemeid_flag:
            ...
            while 1:
                try:
                    ...
                    print('[*] Get zb_zplb aweme_id_list failed. type:%s, num_zb:%s, url_zb:%s at %s' % (
                    type, one_record.num_zb, one_record.url_zb, time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())))
                    time.sleep(5)
        ...
        for aweme_id in aweme_id_list:
            ...
            query_cmd = "list_%s_zplb.select().where(list_%s_zplb.url_works=='%s',list_%s_zplb.time_update.startswith('%s'))" % (
                type, type, url_works, type, today_date)
            if eval(query_cmd):
                continue
            ...
            while 1:
                try:
                    ...
                    print('[*] Get zb_zplb aweme_Detail_url failed. type:%s, num_zb:%s, url_zb:%s at %s' % (type, one_record.num_zb, one_record.url_zb, time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())))
                    time.sleep(5)
                else:
                    break
        ...
        print('[+]',type,'zb_zplb',one_record.num_zb,one_record.name_zb,'zp amount:%d'%len(aweme_id_list),'Done at',time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))